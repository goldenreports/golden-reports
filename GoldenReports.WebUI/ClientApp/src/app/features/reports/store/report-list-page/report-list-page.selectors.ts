...
export class ReportListPageSelectors {
  public static readonly getState = createSelector(
    selectReportFeature,
    (state) => state[ReportListPageStateKey]
  );

  public static readonly getLoadingFlag = createSelector(
    ReportListPageSelectors.getState,
    (state) => state?.loading
  );
  ...
  public static readonly getReports = createSelector(
    ...
    (reports, namespaceId) =>
      reports.filter((x) => x.namespaceId === namespaceId)
  );
  ...
  public static readonly getViewModel = createSelector(
    ...
    (loading, error, reports) => ({ loading, error, reports } as ReportListVm)
  );
}
