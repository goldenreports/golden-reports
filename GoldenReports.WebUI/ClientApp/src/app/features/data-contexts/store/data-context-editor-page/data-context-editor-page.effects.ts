...
export class DataContextEditorPageEffects {
  ...
  pageOpened$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(
        this.store.select(RouterSelectors.getParam('dataContextId'))
      ),
      map(([, dataContextId]) =>
        dataContextId === 'new'
  ...
  submitDataSource$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(([x, namespaceId]) =>
        dataContextActions.creationRequested({
          ...x,
          newDataContext: {
            ...x.newDataContext,
            namespaceId,
      ...
      this.actions$.pipe(
        ...
        map((x) =>
          this.router.navigateByUrl(
            `/namespaces/${x.dataContext.namespaceId}/data-contexts/${x.dataContext.id}`
          )
        )
      ),
    { dispatch: false }
  ...
  dataContextCreationFailed$ = createEffect(() =>
    this.actions$.pipe(
  ...
  submitChanges$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map((x) =>
        dataContextActions.updateRequested({
  ...
  dataContextUpdateFailed$ = createEffect(() =>
    this.actions$.pipe(
      ofType(dataContextActions.updateFailed),
      map((x) =>
        dataContextEditorPageActions.dataContextUpdateFailed({ error: x.error })
      )
    )
  );
}
