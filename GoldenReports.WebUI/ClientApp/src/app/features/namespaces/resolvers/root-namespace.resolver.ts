...
export class RootNamespaceResolver
  implements Resolve<NamespaceDto | undefined>
{
  constructor(
  ...
  resolve(
    ...
    | Observable<NamespaceDto | undefined>
    | Promise<NamespaceDto | undefined>
    | NamespaceDto
    | undefined {
    ...
    return this.store.select(NamespaceSelectors.getRoot).pipe(
      ...
      take(1),
      tap((rootNamespace) =>
        this.router.navigate(['/', 'namespaces', rootNamespace?.id])
      )
    );
  }
}
