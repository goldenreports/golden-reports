...
export class ReportEffects {
  ...
  getNamespaceReports$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ namespaceId }) =>
        this.namespacesService.getNamespaceReports({ namespaceId }).pipe(
          ...
          catchError((resp: HttpErrorResponse) =>
            of(reportActions.namespaceReportsFetchFailed({ error: resp.error }))
          )
        )
      )
    )
  ...
  getReportById$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap((payload) =>
        this.reportsService.getReportById({ reportId: payload.reportId }).pipe(
          map((report) => reportActions.reportFetched({ report })),
          catchError((resp: HttpErrorResponse) =>
            of(reportActions.reportFetchFailed({ error: resp.error }))
          )
        )
      )
    )
  ...
  createReport$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap((payload) =>
        this.reportsService.createReport({ body: payload.newReport }).pipe(
          map((report) => reportActions.reportCreated({ report })),
          catchError((resp: HttpErrorResponse) =>
            of(reportActions.creationFailed({ error: resp.error }))
          )
        )
      )
    )
  ...
  updateReport$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((payload) =>
        this.reportsService
          .updateReport({ reportId: payload.reportId, body: payload.report })
          .pipe(
            map((report) => reportActions.reportUpdated({ report })),
            catchError((resp: HttpErrorResponse) =>
              of(reportActions.updateFailed({ error: resp.error }))
            )
          )
      )
    )
  ...
  deleteReport$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((payload) =>
        this.reportsService.deleteReport({ reportId: payload.reportId }).pipe(
          map(() =>
            reportActions.reportRemoved({ reportId: payload.reportId })
          ),
          catchError((resp: HttpErrorResponse) =>
            of(reportActions.removeFailed({ error: resp.error }))
          )
        )
      )
    )
