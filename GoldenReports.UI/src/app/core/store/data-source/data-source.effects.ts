...
export class DataSourceEffects {
  ...
  createDataSource$ = createEffect(() => this.actions$.pipe(
    ofType(dataSourceActions.creationRequested),
    exhaustMap(payload => this.dataSourcesService.createDataSource({ body: payload.newDataSource }).pipe(
      map(dataSource => dataSourceActions.dataSourceCreated({ dataSource })),
      catchError((resp: HttpErrorResponse) => of(dataSourceActions.creationFailed({ error: resp.error })))
    ))
  ));
  ...
  deleteDataSource$ = createEffect(() => this.actions$.pipe(
    ofType(dataSourceActions.removeRequested),
    mergeMap(payload => this.dataSourcesService.deleteDataSource({ dataSourceId: payload.dataSourceId }).pipe(
      map(() => dataSourceActions.dataSourceRemoved({ dataSourceId: payload.dataSourceId })),
      catchError((resp: HttpErrorResponse) => of(dataSourceActions.removeFailed({ error: resp.error })))
    ))
  ));

  getDataSourceById$ = createEffect(() => this.actions$.pipe(
    ofType(dataSourceActions.dataSourceRequested),
    exhaustMap(payload => this.dataSourcesService.getDataSourceById({ dataSourceId: payload.dataSourceId }).pipe(
      map(dataSource => dataSourceActions.dataSourceFetched({ dataSource })),
      catchError((resp: HttpErrorResponse) => of(dataSourceActions.dataSourceFetchFailed({ error: resp.error })))
    ))
  ));
}
